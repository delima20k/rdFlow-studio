...
class VideoConverterService {
  ...
  async convertWebMToMP4(webmBlob, options = {}) {
    ...
    try {
      ...
      this.ffmpeg.FS('writeFile', inputFileName, await fetchFile(webmBlob));

      // Construir comando FFmpeg baseado nas opções
      const ffmpegArgs = [
        '-r', '30',               // CRÍTICO: Forçar 30 FPS no input (evita duplicação)
        '-i', inputFileName,
      ...
      console.log('[VideoConverter] Iniciando conversão...');
      console.log('[VideoConverter] Argumentos:', ffmpegArgs.join(' '));
      console.log('[VideoConverter] Tamanho entrada:', (webmBlob.size / 1024 / 1024).toFixed(2), 'MB');
      ...
      console.log('[VideoConverter] Compressão:', ((1 - mp4Blob.size / webmBlob.size) * 100).toFixed(1) + '%');
